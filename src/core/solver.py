...
class Solver:
    ...
    def handle_delete_cache_state(self, cache_gs: Game_State):
        """
        Used on *pre-filter* cache. What to do when you want this solver to delete a cache_gs from the cache. Return True if there was an error, otherwise return False. In the case of standard solvers, all cache states are expected to be in the cache, so return True if the cache_gs is not in the cache, otherwise, delete it.
        """
        if cache_gs not in self._evaluations_cache:
            return True # unexpected, return that an error occurred.
    ...
    def handle_get_cache_state_eval(self, cache_gs: Game_State):
        ...
        """
        cost = self._evaluations_cache.get(cache_gs)
        error = (cost is None)
        return (cost, error)
    ...
    def handle_state(self, curr_working_gs, curr_cache_gs, gs_to_put_in_cache, stack, new_ev_cache):
        if not self.exist_moves(curr_working_gs):
            error_occurred = self.handle_delete_cache_state(curr_cache_gs)
            if error_occurred:
                message = "The following state in handle_state was expected to be in cache, but it wasn't."
    ...
    def _handle_state_helper(self, curr_working_gs, curr_cache_gs, gs_to_put_in_cache, stack, new_ev_cache):
        (gs_eval_before, error) = self.handle_get_cache_state_eval(curr_cache_gs)
        if error:
            message = "The following state in handle_state was expected to be in cache, but it wasn't."
            self._filter_cache_error_show(curr_working_gs, curr_cache_gs, message)
        self.handle_delete_cache_state(curr_cache_gs)
        gs_eval_now = self._filter_calculate_best_move(curr_working_gs)
        # NOTE: may need to use floating point 'equal' here
        if ((gs_eval_before is not None) and (gs_eval_now != gs_eval_before)):
            console.print("current_gs_eval:", gs_eval_now)
            console.print("previous_gs_eval:", gs_eval_before)
            console.print(
                "[pink]NOTE[/pink]: If the above two values are very close, should use floating point 'equal' here."
            )
            message = (
                "You should keep track of a state to parent[] list dict and update all parents' evaluations now and start over with filter cache if this is a depth pruning thing."
            )
            self._filter_cache_error_show(curr_working_gs, curr_cache_gs, message)
        new_ev_cache[gs_to_put_in_cache] = (self.best_move, gs_eval_now)
